...
## 2026-07-18 W2遠隔売却+W4傭兵所在町+監査低優先3件（Codex Terra実装、セーブv27→v28）

- **W2 全町倉庫ビュー+遠隔売却（v27）**: 新規`RemoteSaleManager`。商会組合「全町倉庫」オーバーレイから他町倉庫の閲覧と売却注文（アイテム数量/装備個体）。注文時に対象町倉庫から予約差引、キャンセルで返却。約定=現在町との距離日数（最低1日）後、**約定日のその町需要込み売値**でAddGold（装備は品質・強化倍率込み）。日次リザルト表示。テスト7件（初回未作成を1往復で追完）。
- **W4 傭兵の所在町（v28）**: `MercenaryInstance.CurrentTownIndex`。雇用=現在町、編成は現在町の傭兵のみ（「◯◯は別の町にいます」表示）、`TownProgressState.SetCurrentTown`でパーティー全員同期（街道勝利・中央島航路両対応）、輸送護衛は出発町限定→到着で目的地へ、遠征呼び戻しでnearbyTownへ、治療は現在町のみ。商会一覧に所在表示。v27以前の傭兵はcurrentTownIndexへ移行。テスト5件。
- **監査低優先3件**: 採取イベントを地域別素材化（東方=鉄/薬草/堅木、北西=銀/霊木、南西+1個、中央島+2個。12ケースのピン留めテスト）／消耗品9種の説明にリーフ製薬文化を反映／魔物図鑑の等級補助表記（最強/上位/最弱）+カテゴリ日本語化。内部識別子`Guild`リネームはPrefabパス破損リスクのため見送り（記録のみ）。
- 全段階build警告0・エラー0。**Unity確認項目**: Ctrl+R→EditMode全件（新テスト12件超）→全町倉庫ビューの表示と遠隔売却の流れ→他町傭兵の編成不可と所在表示→採取イベントの地域素材→旧セーブ(v26以前)読込で傭兵所在が現在町になること。確認後コミット推奨。

### 2026-07-18 追記: 鍛冶レシピから売却用素材を除去（Terra、ユーザー指摘）

- 分類=売却用のGoblin Ear/Bat Wingがレシピ素材に残存していた矛盾を解消。該当16レシピの素材をCursed Bone/Monster Fangへコスト水準を保って置換（魔石要件は不変）。全レシピの素材がCraftingMaterialであることの回帰テストをTradeMaterialTestsへ追加。SellOnly GUIDのレシピ参照0件確認、build 0/0。

### 2026-07-18 追記: ドロップの種族準拠再設計+レシピ素材是正（Terra、ユーザー指摘）

- ユーザー指摘（オーガメイジ→核、レイス→骨、耳/翼/牙の種族不一致等）を受け、全敵ドロップを「その生き物から採れるもの」へ全面置換。新規素材11種（粘液/ウサギの角/毛皮/クモの糸/甲虫殻/鱗粉/霊魂の残滓/トカゲ鱗/トロル皮/オーガ血石/黒鉄鉱片、価格は等級帯水準）。耳=ゴブリン系のみ、翼=コウモリのみ、骨=スケルトン系のみ、牙の低級供給は野犬・コボルト。ヴェルム系=黒鉄鉱片（工業都市テーマ）。薬材はリーフ需要1.25対象へ。全レシピの素材入手可能性テスト+種族対応ピン留めテスト追加。docs/ITEM_DROP_TABLE.md再生成。build 0/0。
- 先行変更（16レシピの売却用素材除去）と合わせ、分類・生態・入手経路が一貫した状態。Unity EditMode全緑確認後にコミット推奨。

## 2026-07-21 家側・専用限定装備 段階1（新規4ダンジョン、Codex Terra）

- 全ダンジョン専用限定装備実装の段階1完了。ユーザー確定: セット効果は全14セット、装備名称はSol案を仮採用。
- 新規4ダンジョン(翠樹族の集落跡R6/竜鱗峡谷R7/熔炉防衛区R8/奈落境門R9)の汎用NormalRank流用を解消し、専用ItemDataSO 12点(各3部位)を新規作成。基礎ステータスは各Rank規定値どおり、差別化は特殊効果/特攻+セット効果。旧汎用SOは残置(ドロップ参照から外すのみ)。
- 効果割当: 翠樹族=武器LowHp0.12/防具軽減0.07/装飾再生0.02、竜鱗峡谷=武器竜特攻0.14/防具開幕防0.10/装飾開幕攻0.08、熔炉=武器構造体特攻0.16/防具軽減0.10/装飾開幕防0.12、奈落境門=武器悪魔特攻0.18/防具開幕防0.15/装飾竜特攻0.15。
- EquipmentSetIdに4セット追加、2部位/3部位ボーナス実装(R6=HP15/攻2防2速1%、R7同、R8=HP20/攻3防2速1.5%、R9=HP25/攻3防3速2%)。
- docs/IMAGE_ASSET_LIST.mdへ専用装備12点の図鑑画像(未配置)追記。テスト追加(装備構成/許可帯/セット集計)。build警告0エラー0、GUID/persistentId重複0。EditMode全緑ユーザー確認済み。
- 残段階: ②低級共有セット3ダンジョン(はじまり/獣道/地下水路 R3)分離 → ③廃坑2(廃坑/旧採石場 R4)分離 → ④既存専用5整理 → ⑤黒鉄坑/黒土深淵数値調整 → ⑥セット効果総合調整。

## 2026-07-21 家側・専用限定装備 段階2〜6完了（Codex Terra実装/Sol最終監査）

- 段階2: 低級共有「鬼狩り系」を はじまりの洞窟/樹海の獣道/地下水路 の専用9点(R3)へ分離。3セットID追加(2部位HP+5/3部位攻1防1)。獣道=獣特攻0.08、地下水路=人型特攻0.08+開幕防0.05。
- 段階3: 「採掘系」を 封じられた廃坑/旧採石場 の専用6点(R4)へ分離。2セットID追加(2部位HP+10/3部位攻2防1)。旧採石場=不死特攻0.10。
- 段階4: 既存専用5ダンジョン(古代遺跡/樹冠迷宮/天嶺砦/黒鉄坑/黒土深淵)の既存装備15点へ効果/セットIDを整理(基礎ステータス・名称・persistentIdは不変)。ExistingEquipmentEffectsを正本に一元管理し二重付与防止。黒土深淵のlimitedEquipmentDropsから汎用NormalRank09を除外し固有3点(AbyssFang/AbyssMantle/AbyssSeal)のみに。
...
| 指標 | 開始 | 完了 |
|---|---|---|
| partial | 19本・8,627行 | **5本・2,991行**（65%減） |
| 抽出クラス | 0 | **19** |
