...
- 最初にUnity Test Runner全件と、最新の静止画戦闘演出を通常戦・ボス戦・街道戦で確認する。
- 確認対象は、戦闘開始表示、スキル名、対象枠、HP補間、クリティカル振動、3対5表示、1x／2x／4x、結果までスキップ。
- その後、試作用モンスター4枚と背景3枚を追加してレイアウトを確定する。
- 大量画像生成、BGM・環境音、大規模な敵・バランス生成は着手前にユーザー承認を得る。
- 最新C#ビルドは警告0・エラー0。直近変更後のUnity Test Runnerと実画面確認は未実施。
