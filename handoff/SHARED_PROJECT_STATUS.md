...
## 2026-07-12 ポートフォリオ提出前評価とフェーズS追加

- 本作がポートフォリオであることを踏まえた提出前リスク評価を実施。権利面の結論: 画像はChatGPT生成（ユーザー帰属・商用可・問題なし）、フォント同梱はZen Kurenaido（OFL、ライセンス文同梱済み）のみで問題なし。
- **`CLAUDE_WORK_LOG.md`の第2次計画に「フェーズS（提出前仕上げ）」を追加**: S-1 README更新（完了: AI活用の実態明記+アセット出所セクション追加）、S-2 ダンジョンUIバグ修正、S-3 タイポ/文字化け/companyName、S-4 審査者向け導線+サンプルセーブ、S-5 ビルドのライセンス表記、S-6 設計意図の整理。提出前はフェーズSを最優先とする。

## 2026-07-13 フェーズA・B完了、フェーズC着手前

- **フェーズA（衛生・小規模6件）・フェーズS（提出前仕上げ）は全項目完了し、ユーザーがUnity上で動作確認済み**（残るユーザー作業はS-4サンプルセーブ配置とS-6設計意図の整理のみ）。
...
## 2026-07-22 家側・提出前検証とUX改善バックログ受領

### 提出前検証（実施済み）
- Unity Editorを開いたまま非破壊クローン上でテスト実行。**EditMode 559件中557成功・失敗0・スキップ2、PlayMode 8/8成功。合計567件中565成功・失敗0**。スキップ2件は`Explicit`属性の手動実行専用アセット検証で不具合ではない。以前失敗報告のあった`LowHpDamageBonus_ActivatesBelowButNotAtThreshold`は現在Passed。
- **セーブ整合性監査: 新装備・特殊能力の消失リスクなし**。ItemDataSO 226件すべてに`persistentId`あり・重複なし。特殊能力は`ItemDataSO.equipmentEffects`(マスタ側)に保持され`EquipmentInstance`へ複製されないため、セーブDTO変更もバージョン更新も不要。`CurrentVersion=28`と`SaveDataMigrator`は整合。
- **潜在リスク: `EnemyDataSO` 54件が`persistentId`未設定**。現在はアセット名フォールバックで動作し実効ID重複もないが、**ID付与が完了するまで対象アセット名を変更しないこと**。
- **Windowsビルドは未検証**。クローンのパスがMAX_PATH(260文字)を超過しDLLを開けず失敗した環境制約で、本体プロジェクト(該当パス183文字)では発生しない。コンパイルエラーは0件。Build Settingsからの手動ビルドが必要。
- 新規: `TEST_BASELINE.md` / `MANUAL_TEST_CHECKLIST.md`(プロジェクト直下)。`README.md`は既存内容を保持したまま、実在しない`SampleSave/game-save.json`の「同梱予定」記述を削除し、テスト件数を実測値へ修正、既知の問題・テスト状況・起動方法を追記。

### UX改善バックログ（未着手）
- ユーザーから改善案15件を受領し `handoff/UX_IMPROVEMENT_BACKLOG.md` へ記録した。施設説明のオンボーディング、治療院/転職の情報表示、倉庫の一括売却・ツールチップ・装備保管枠、鍛冶屋の段階化、市場/鍛冶屋の画像表示、日次リザルトの色分け、町マップ画像の個別化など。
- 全件未着手。着手前にユーザー確認が必要な項目あり(#1と#15の方式選択、#9「奥に進む」の具体化、#11の倉庫容量計算方針)。#11と#15はセーブ構造へ波及する可能性があるため要事前判定。
- **提出前検証(ビルド・手動確認・スクリーンショット/動画)が優先**。本バックログは提出後または余力がある場合に着手する。
- 追加受領(同日): **#16 装備以外のアイテムの図鑑をメニューに追加**、**#17 タイトル画面に戻るボタンをメニューに追加**。計17件。#16はアイテム用の発見管理が既存に無ければセーブ構造追加(v28→v29)を伴うため要事前判定。

## 2026-07-22 家側・強化鉱石の入手経路調査とバランス懸念（要判断）

- ユーザー質問を受けて「低級強化鉱石」(`item.EnhancementOre`)の入手経路を全調査。**入手経路はダンジョン踏破報酬(`clearItemRewards`)のみ**と確定。敵ドロップ・ボスドロップ・市場・鍛冶屋購入・鉱脈イベント・共通ドロップルール・遠征のいずれからも入手できない。
...
## 2026-07-31 家側・UX改善バックログ全17件対応完了 + 基準の訂正

- **UX改善バックログの実コード棚卸しを実施**し、大半が昨日までの実装で既に完了済みと判明(当初「全件未着手」は誤り)。残った#1(部分)・#5(部分)・#16(未着手)を実装し、**全17件対応済み**に。詳細は`handoff/UX_IMPROVEMENT_BACKLOG.md`と`CLAUDE_WORK_LOG.md`。
  - #1: 新規ゲーム時のみチュートリアル自動表示(セーブ変更なし)。
  - #5: 一括売却を「売却用＋環境素材6種」へ拡張(制作材料は除外)。
  - #16: メニューに「アイテム図鑑」新設(入手済のみ表示)。発見管理のため**セーブv36→v37**へ昇格(`discoveredItemPersistentIds`追加、移行ステップ＋移行テスト付き)。
- 検証: Runtime/EditModeテスト両csprojとも`dotnet build`警告0・エラー0。luna裏取りでセーブ整合・挙動不変・範囲を確認。**Unity Test Runnerでの全件PASS確認は要ユーザー**(特にv36→v37移行テスト)。
- **⚠ 基準の訂正**: 実コードの`CurrentVersion`は本実装後**v37**(セッション前半に私が記録した`TEST_BASELINE.md`等の「v28」「EditMode 559件」は同期前の古いスナップショット値で陳腐化)。正しい現状はEditMode≒794件+新規、UI層はD→B−〜Bへリファクタ済み(`handoff/UI_REFACTOR_HANDOFF.md`が正本)。`TEST_BASELINE.md`の再計測が望ましい。
...
## 2026-07-27 家側・提出前チェックリストを作成
- handoff/SUBMISSION_CHECKLIST.md を新設。提出(GitHub公開/ビルド配布/応募)前に確認する項目を体系化。
- 内容: 前提確認 / 未コミット作業の整理 / 機密・個人情報の除外(handoff公開判断含む) / ライセンス / ビルド動作確認 / README見せ方(スクショ追加推奨) / プレイ体験 / 提出直前手順 / 未解決の技術課題。
