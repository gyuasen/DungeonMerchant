...
> 作成日: 2026-07-31（当初 2026-07-22 と誤記）
> 精査体制: sol(起草)/ luna(裏取り検証)/ メイン(統括監査)
> 対象: `Assets/Proiject/` 配下(Scripts / Editor / Tests)。Unity生成物・パッケージは対象外。

> ## ⚠ 状態の訂正（2026-07-31）
> **本レポートの本文は、同期前の古いスナップショットに対して精査したもので、一部は既に解消済み。** 以下が 2026-07-31 に実コードで再確認した現状。着手前に必ず参照すること。
>
> | 指摘 | 当時 | 現状（2026-07-31 再確認） |
> |---|---|---|
> | セーブを原子的にする（高） | 直接上書き | **解消済み**。`.tmp`→`File.Replace`（既存を `.bak` 退避）／`File.Move` による原子的置換（`SaveManager.cs:118-141`） |
> | `SimpleMercenaryHireUI` の分割（高） | 本体1,129行の巨大partial群 | **大幅に実施済み**。19→10 partial・合計約7,839行（本体1,459行）、View/Presenter 10クラス抽出、UI層評価 D→B−〜B（`handoff/UI_REFACTOR_HANDOFF.md`が正本） |
> | `FindObjectOfType` の集約（高） | 134箇所 | **一部実施**。101箇所まで削減（同上ハンドオフ） |
> | ストレージ制限の fail-open 修正（高） | 未対応 | **未対応（依然有効）**。`CanStore()` は依存 null 時に `true`（`ProgressionManager.cs:99-104`） |
> | セーブ関連の基準値 | `CurrentVersion=28` / `ItemDataSO` 226件 / `EnemyDataSO` 未設定54件 | **`CurrentVersion=37` / `ItemDataSO` 217件 / `EnemyDataSO` 未設定53件（総99）** |
>
> 上表以外の中・低優先項目（`Resources.LoadAll` キャッシュ化、ツールチップ毎フレーム生成、乱数注入、`SaveManager` 分割、Bootstrap 購読順 等）は 2026-07-31 時点で未再確認。着手前に現行コードで再検証すること。以下の本文は当時の分析として残す。
...
- **【2026-07-31: 大幅に解消済み。19→10 partial・合計約7,839行へ縮小、View/Presenter 10クラス抽出済み。以下は当時の分析】**
...
- **【2026-07-31: 解消済み。`.tmp`→`File.Replace`（既存を `.bak` 退避）／`File.Move` による原子的置換を実装（`SaveManager.cs:118-141`）。以下は当時の分析】**
...
> **【2026-07-31 訂正】** 下記のうち #2(セーブ原子化)は解消済み、#4(UI分割)・#5(FindObjectOfType集約)は大幅に実施済み。**現時点で残る主な着手候補は #1(fail-open 修正)と #3(Bootstrap 購読順)**。以下は当時の並べ替え。

1. **ストレージ fail-open 修正**(工数小/確実な不具合)— 数行で潰せる。容量無制限バグは実プレイに直結。**←現状も未対応・最優先候補**
2. ~~**セーブ原子化 + 破損復旧**~~ — **解消済み**(`SaveManager.cs:118-141`)。
3. **Bootstrap 初期化・購読順の固定**(工数中)— 再現性の低い購読漏れ不具合を解消。DI移行の足場。
4. ~~**SimpleMercenaryHireUI の段階分割**~~ — **大幅に実施済み**(10 partial・View/Presenter抽出)。
5. ~~**FindObjectOfType の Composition Root 集約**~~ — **一部実施済み**(134→101箇所)。
