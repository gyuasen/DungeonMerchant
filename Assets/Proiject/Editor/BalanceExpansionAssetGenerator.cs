...
public static class BalanceExpansionAssetGenerator
{
    ...
    static void BuildEquipment(BalanceExpansionEquipmentDefinition definition)
    {
        ...
        item.itemName = definition.JapaneseName;
    ...
    static void ApplyEquipmentStats(ItemDataSO item, BalanceExpansionEquipmentDefinition definition)
    {
        ...
        if (definition.Slot == EquipmentSlot.Weapon) { item.bonusAttack = rank * 3 + (definition.ClassIndex == 0 ? 2 : 0); item.bonusMaxHP = definition.ClassIndex == 0 ? rank * 4 : 2; item.bonusAttackSpeed = definition.ClassIndex == 1 ? .03f : 0f; item.basePrice = 180 + rank * 95; return; }
        if (definition.Slot == EquipmentSlot.Armor) { item.bonusMaxHP = rank * 9 + (definition.ClassIndex == 0 ? rank * 2 : 0); item.bonusDefense = rank * 2 + (definition.ClassIndex == 0 ? 2 : 0); item.bonusAttack = definition.ClassIndex == 2 ? 2 : 0; item.basePrice = 255 + rank * 55; return; }
        item.bonusMaxHP = rank * 3 + (definition.ClassIndex == 0 ? rank : 0);
        item.bonusAttack = definition.ClassIndex == 2 ? rank + 2 : definition.ClassIndex == 0 ? 2 : 3;
        item.bonusDefense = definition.ClassIndex == 0 ? rank : 1;
        item.bonusAttackSpeed = definition.ClassIndex == 1 ? .02f + rank * .005f : definition.ClassIndex == 2 ? .01f + rank * .003f : .01f;
