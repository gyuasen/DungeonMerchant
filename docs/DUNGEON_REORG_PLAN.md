...
- 装備ランク規則は「市場=町ランク、鍛冶=町ランク+1、ダンジョン限定装備の許可帯=町ランク〜町ランク+2」（通常町は上限Rank 9、隠し島はRank 10のみ）。`GetDungeonEquipmentRank` は各町の標準/最高ランク（町+2）を表し、複数ダンジョンの限定装備はこの許可帯内に置く。黒鉄坑はHighest化後も Rank 8 を維持し、Rank 9 はアビス専用。
...
- はじまりの洞窟 (6): `EnemyData`, `Grade10GreenSlime`, `Grade10BlueSlime`, `Grade10MossSlime`, `Grade10HornRabbit`, `Grade09WildDog`。スライム4+角ウサギ+野犬で統一する。
...
- 地下水路 (9): `Grade09Goblin`, `Grade09GoblinScout`, `Grade09GoblinSpearman`, `enemy_job_goblin_assassin`, `enemy_job_goblin_magician`, `enemy_job_goblin_knight`, `enemy_job_goblin_raider`, `enemy_job_goblin_veteran`, `enemy_slime_slime_acid`。酸液スライムは地下水路専属とする。はじまりの洞窟は上記のスライム4種構成を採用し、酸液・毒液スライムは置かない。

- 封じられた廃坑 (8): `Grade07Zombie`, `Grade07BoneHound`, `Grade08CaveBat`, `Grade08GiantRat`, `Grade08CaveSpider`, `Grade08VenomMoth`, `Grade08RockBeetle`, `enemy_slime_slime_venom`。洞窟獣を主軸に不死は少数とし、`Grade07Skeleton` と `Grade07ArmoredSkeleton` は旧採石場専属にする。
...
- 霧の古代遺跡 (6): `Grade05StoneGolem`, `Grade05IronGolem`, `Grade05OgreMage`, `Grade06Hobgoblin`, `Grade06Troll`, `Grade06MarshLizard`。`Grade06Lizardman` と `Grade06Orc` は置かない。
...
- 樹冠迷宮 (5): `Grade04DarkMage`, `Grade05IronGolem`, `Grade05StoneGolem`, `enemy_slime_slime_thunder`, `Grade06Orc`。`Grade03GlaadSkyWarden` と `VelmEmberforgedAutomaton` は配置しない。
- 天嶺砦 (10): `Grade03Wyvern`, `Grade03GlaadFrostDrake`, `Grade03GlaadSkyWarden`, `Grade04GlaadGaleHarpy`, `enemy_job_wyvern_hexer`, `enemy_job_wyvern_skyrider`, `enemy_job_wyvern_ironwing`, `enemy_job_wyvern_ravager`, `enemy_job_wyvern_captain`, `enemy_slime_slime_frost_crystal`。
...
- 熔炉防衛区 (6): `Grade05IronGolem`, `Grade05StoneGolem`, `Grade04DarkMage`, `enemy_slime_slime_frost_crystal`, `Grade07Wraith`, `Grade05OgreMage`。大熔炉巨像を含む構造体主軸を維持し、`VelmEmberforgedAutomaton` は黒鉄坑専属、`Grade03GlaadSkyWarden` は天嶺砦専属にする。
...
- ボス種族の例外は変更しない。はじまりの洞窟・樹海の獣道・封じられた廃坑は導入部の人型オーガ系ボス、天嶺砦は城主の人型ボスを物語上の例外として維持する。
...
5. 解決済み: `VelmEmberforgedAutomaton` は黒鉄坑専属。熔炉防衛区は既存の石・鉄ゴーレムと大熔炉巨像で構造体主軸を維持する。
